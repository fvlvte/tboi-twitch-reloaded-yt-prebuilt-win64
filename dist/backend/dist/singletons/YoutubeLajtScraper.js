...
class YoutubeLajtScraper {
    static YT_CHAT_SWITCH_SELECTOR = "yt-icon[icon='expand']";
    static YT_LIVE_CHAT_SELECTOR = "tp-yt-paper-listbox[id='menu'] a[aria-selected='false'] tp-yt-paper-item tp-yt-paper-item-body";
    page;
    ytId = "";
    constructor(page) {
        this.page = page;
        ...
        for (const item of messageArray) {
            if (item.addChatItemAction) {
                if (item.addChatItemAction.item.liveChatMembershipItemRenderer) {
                    ...
                    _1.HttpServer.getInstance().putMessage(this.ytId, {
